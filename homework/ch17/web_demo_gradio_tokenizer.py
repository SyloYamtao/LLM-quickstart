...
# Set the default model path from environment variables
# MODEL_PATH = os.environ.get('MODEL_PATH', '/root/dataDisk/models/chatglm3-6b')
MODEL_PATH = os.environ.get('MODEL_PATH', '/root/dataDisk/code-workspace/ChatGLM3/finetune_demo/output/checkpoint-3000')

TOKENIZER_PATH = os.environ.get("TOKENIZER_PATH", MODEL_PATH)
# /root/dataDisk/code-workspace/ChatGLM3/finetune_demo/output/checkpoint-3000
...
def load_model_and_tokenizer(
    ...
    Args:
        model_dir (Union[str, Path]): The directory containing the model and tokenizer.
        trust_remote_code (bool): Whether to trust remote code.

    Returns:
        tuple[ModelType, TokenizerType]: The loaded model and tokenizer.
    """
...
# Load the model and tokenizer using the specified paths
...
class StopOnTokens(StoppingCriteria):
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor, **kwargs) -> bool:
        stop_ids = [0, 2]  # Define the stop tokens
...
def parse_text(text: str) -> str:
    """
    Parse and format text for display in the chat interface.
...
def predict(history, max_length, top_p, temperature):
    """
    Generate a prediction from the model based on the conversation history.
    ...
    Yields:
        list: The updated conversation history with the model's response.
    """
    stop = StopOnTokens()
    messages = []
...
with gr.Blocks() as demo:
    ...
    def user(query, history):
        ...
        Returns:
            tuple: An empty string and the updated conversation history.
        """
